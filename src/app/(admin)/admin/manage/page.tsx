...
export default function ManageContentPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8 font-sans bg-white dark:bg-slate-950 min-h-screen">

      {/* HEADER */}
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          <h1 className="text-4xl font-black font-serif text-slate-900 dark:text-white tracking-tight">Βάση Γνώσεων</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium mt-1">Διαχείριση Περιεχομένου: <span className="text-blue-600 font-bold">{currentConfig.label}</span></p>
        </div>
        <div className="flex w-full lg:w-auto gap-3">
          ...
          <button onClick={fetchData} className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-2xl transition-all text-slate-400 hover:text-slate-900 dark:hover:text-white shadow-sm">
      ...
      {/* NAVIGATION GRID */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 bg-slate-50 dark:bg-slate-800/50 p-6 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm">
      ...
      <div className="space-y-6">
        <div className="relative group">
          ...
          <input
            ...
            className="w-full pl-14 pr-6 py-5 rounded-3xl border border-slate-200 dark:border-slate-700 focus:ring-4 focus:ring-blue-50 focus:border-blue-300 outline-none font-bold text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-900 shadow-sm transition-all"
          />
        </div>
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50/50 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  <tr>
                  ...
                  {filteredItems.map((item, index) => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                        ...
                        <td className="p-5">
                          <div className="text-base font-serif font-bold text-slate-800 dark:text-slate-200 leading-snug line-clamp-2">
                            {item.title}
                          </div>
                        ...
                        <td className="p-6 text-right">
                          <div className="flex justify-end gap-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-all">
                            <Link
                              ...
                              className="p-3 text-slate-400 hover:text-blue-600 hover:bg-white dark:hover:bg-slate-800 rounded-xl shadow-sm border border-transparent hover:border-slate-100 dark:hover:border-slate-700 transition-all"
                            >
                              <Edit className="h-5 w-5" />
                            </Link>
                            <button
                              onClick={() => handleDelete(item.id)}
                              className="p-3 text-slate-400 hover:text-red-600 hover:bg-white dark:hover:bg-slate-800 rounded-xl shadow-sm border border-transparent hover:border-slate-100 dark:hover:border-slate-700 transition-all"
                            >
              ...
              {filteredItems.length === 0 && (
                <div className="p-10 text-center text-slate-400 dark:text-slate-500 font-medium">Δεν βρέθηκε τίποτα.</div>
              )}
            </div>
          )}
        </div>
  ...
  return (
    <button
      ...
      className={`flex items-center justify-between p-3 rounded-2xl border-2 transition-all ${isActive
          ? 'bg-white dark:bg-slate-800 border-slate-900 dark:border-blue-500 shadow-lg scale-105 z-10'
          : 'bg-white/50 dark:bg-slate-800/30 border-transparent text-slate-400 hover:bg-white dark:hover:bg-slate-800 hover:border-slate-200 dark:hover:border-slate-700'
        }`}
    >
      <div className="flex items-center gap-3">
        <div className={`p-2 rounded-xl ${isActive ? cat.bg : 'bg-slate-100 dark:bg-slate-700'}`}>
          <Icon className={`h-5 w-5 ${isActive ? cat.color : 'text-slate-300 dark:text-slate-500'}`} />
        </div>
        <span className={`text-[11px] font-black uppercase tracking-tighter truncate ${isActive ? 'text-slate-900 dark:text-white' : 'dark:text-slate-400'}`}>
          {cat.label}
        </span>
      </div>

      <div className={`text-[10px] font-black px-2 py-0.5 rounded-lg ml-2 ${isActive ? 'bg-slate-900 dark:bg-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-400 dark:text-slate-400'
        }`}>
