...
export default function AdminDashboard() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950"><Loader2 className="animate-spin h-8 w-8 text-blue-500" /></div>;
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-10 font-sans">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-900 dark:text-white font-serif tracking-tight">Πίνακας Ελέγχου</h1>
            <p className="text-slate-500 dark:text-slate-400 font-medium">Διαχείριση βάσης γνώσεων και θεωρίας</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {cards.map((card, i) => (
          <Link key={i} href={card.link} className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group">
            ...
            <div className="text-5xl font-black text-slate-900 dark:text-white mb-1 font-serif tracking-tight">{card.count}</div>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            {categories.map((cat, i) => (
              <div key={i} className="bg-slate-50/50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 flex items-center gap-5">
                <div className={`p-4 rounded-2xl shadow-sm ${cat.bg}`}>
                  <cat.icon className="h-6 w-6" />
                </div>
                <div>
                  <div className="text-2xl font-black text-slate-900 dark:text-white font-serif">{cat.count}</div>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm">
          ...
          <div className="space-y-4">
            {languageStats.map((item, i) => (
              <div key={i} className="flex items-center justify-between p-5 bg-slate-50/50 dark:bg-slate-800/50 rounded-2xl group hover:bg-white dark:hover:bg-slate-800 transition-all">
                <div className="flex items-center gap-4">
                  <item.icon className={`h-5 w-5 ${item.color}`} />
                  <span className="font-bold text-slate-700 dark:text-slate-300">{item.title}</span>
                </div>
                <span className="text-xl font-black text-slate-900 dark:text-white font-serif">{item.count}</span>
              </div>
            ))}
          </div>
        </div>
