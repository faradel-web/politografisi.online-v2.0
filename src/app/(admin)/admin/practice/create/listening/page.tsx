...
function EditorContent() {
  ...
  const handleJsonImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!theme) return alert("Λανθασμένη μορφή JSON");
      ...
      } catch (err) { alert("Σφάλμα JSON: " + err); }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title || !audioUrl) return alert("Συμπληρώστε τον τίτλο και τον ήχο.");
