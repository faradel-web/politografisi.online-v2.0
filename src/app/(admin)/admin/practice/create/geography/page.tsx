...
const PAGE_TITLE = "Γεωγραφία (Updated Editor)";
...
const EDITOR_MODES = [
    ...
    { id: 'MAP', label: 'Διαδραστικός Χάρτης', icon: MapIcon }, // Специфічний тип
];

export default function GeographyEditorPage() {
    return (
...
function EditorContent() {
    ...
    const [mapTolerance, setMapTolerance] = useState<number>(30);
            ...
            try {
                if (editId) {
                    ...
                    if (snap.exists()) {
                        ...
                        if (type.includes('map') || data.points) type = 'MAP'; // Пріоритет карті
                        else if (type === 'multiple-choice') type = 'SINGLE';
                        else if (type === 'multiple-choice-multiple') type = 'MULTI';
                        else if (type === 'matching') type = 'MATCHING';
                        else if (type.includes('fill')) type = 'FILL_GAP';
                        else if (type.includes('true')) type = 'TRUE_FALSE';
                        else if (type.includes('open')) type = 'OPEN';
                        ...
                        if (type === 'MAP') {
                            ...
                            else if (data.correctAnswer && data.correctAnswer.lat) {
                                // Legacy single point migration
                                setMapPoints([{
                                    id: '1',
                                    lat: data.correctAnswer.lat,
                                    lng: data.correctAnswer.lng,
                                    label: "Σημείο 1"
                                }]);
                            }
                            setMapTolerance(data.tolerance || 30);
                        }
                        ...
                        if (type === 'TRUE_FALSE') {
                            if (data.items) setTfItems(data.items);
                                ...
                                setTfItems(data.statements.map((s: string, i: number) => ({
                                    text: s, isTrue: data.correctBooleans ? data.correctBooleans[i] : true
                                })));
                            }
                        }
                    ...
                    if (!snap.empty) {
                        const nextOrder = snap.docs[0].data().order + 1;
                        setOrder(nextOrder);
                        // Авто-перемикання на карту для певних номерів (як у старому коді)
                        if (nextOrder >= 51 && nextOrder <= 70) {
                            setMode('MAP');
                            setQuestionText("Τοποθετήστε τα σημεία στον χάρτη...");
                        }
                    }
                }
    ...
    const handleMapClick = (coords: { lat: number, lng: number }) => {
        if (!currentLabel.trim()) return alert("Εισαγάγετε το όνομα του σημείου!");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const baseDoc: any = {
                ...
                imageUrl: mode === 'MAP' ? null : (imageUrl || null),
                updatedAt: serverTimestamp(),
                category: 'geography'
            };

            if (mode === 'MAP') {
                if (mapPoints.length === 0) return alert("Προσθέστε τουλάχιστον ένα σημείο!");
                baseDoc.points = mapPoints;
                baseDoc.tolerance = mapTolerance;
            }
            else if (mode === 'SINGLE') {
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 font-sans pb-40 transition-colors">
            <div className="max-w-6xl mx-auto">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300 px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest">{COLLECTION_NAME}</div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {/* DYNAMIC FIELDS */}
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-700 min-h-[300px]">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    <div className="space-y-6">
                                        <div className="bg-blue-50 dark:bg-blue-900/30 p-4 rounded-xl border border-blue-100 dark:border-blue-800">
                                            <h3 className="font-bold text-blue-900 dark:text-blue-300 mb-2 flex items-center gap-2"><MapPin size={16} /> Προσθήκη σημείου</h3>
                                            <input value={currentLabel} onChange={(e) => setCurrentLabel(e.target.value)} placeholder="Όνομα (π.χ. Αθήνα)" className="w-full p-3 bg-white dark:bg-slate-800 border border-blue-200 dark:border-blue-700 rounded-lg font-bold outline-none dark:text-white dark:placeholder-slate-500" />
                                            <p className="text-xs text-blue-600 dark:text-blue-400 mt-2">Εισαγάγετε το όνομα και κάντε κλικ στον χάρτη.</p>
                                        </div>

                                        <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 max-h-[300px] overflow-y-auto">
                                            <h3 className="font-bold text-slate-700 dark:text-slate-300 mb-2">Σημεία ({mapPoints.length})</h3>
                                            {mapPoints.map((pt, i) => (
                                                <div key={i} className="flex items-center justify-between bg-white dark:bg-slate-700 p-2 rounded-lg border border-slate-100 dark:border-slate-600 mb-1">
                                                    <span className="font-bold text-sm text-slate-800 dark:text-white">{pt.label}</span>
                                                    <button type="button" onClick={() => setMapPoints(mapPoints.filter((_, idx) => idx !== i))} className="text-red-400"><Trash2 size={16} /></button>
                                                </div>
                                            ))}
                                        </div>

                                        <div className="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-xl border border-orange-100 dark:border-orange-800">
                                            <label className="text-xs font-bold text-orange-800 dark:text-orange-300 block mb-1">Ακτίνα σφάλματος (χλμ)</label>
                                            <input type="number" value={mapTolerance} onChange={(e) => setMapTolerance(Number(e.target.value))} className="w-full p-2 bg-white dark:bg-slate-800 rounded-lg font-bold text-center border border-orange-200 dark:border-orange-700 outline-none dark:text-white" />
                                        </div>
                                    </div>

                                    <div className="lg:col-span-2 h-[500px] border-4 border-slate-100 dark:border-slate-700 rounded-[2rem] overflow-hidden relative">
                                        <GreeceMap markers={mapPoints} onSelect={handleMapClick} />
                                    </div>
                                </div>
                            </div>
                        )}
