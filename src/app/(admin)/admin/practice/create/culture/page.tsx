...
const EDITOR_MODES = [
    ...
    { id: 'FILL_GAP', label: 'Κενά / Inline', icon: AlignLeft },
...
function EditorContent() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        } catch (e: any) { alert("Σφάλμα: " + e.message); }
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 font-sans pb-40 transition-colors">
            <div className="max-w-6xl mx-auto">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="bg-pink-100 dark:bg-pink-900/40 text-pink-800 dark:text-pink-300 px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest">{COLLECTION_NAME}</div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-700">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-2">
                                ...
                                <input value={imageUrl} onChange={e => setImageUrl(e.target.value)} placeholder="URL εικόνας" className="flex-1 bg-transparent outline-none text-sm dark:text-slate-300 dark:placeholder-slate-500" />
                            </div>
                        </div>
                    </div>
                    ...
                    {/* DYNAMIC FIELDS */}
                    <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-700 min-h-[300px]">
                            ...
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {options.map((opt, i) => {
                                        ...
                                        return (
                                            <div key={i} className={`relative p-4 rounded-2xl border-2 transition-all ${isSelected ? 'border-emerald-400 dark:border-emerald-500 bg-emerald-50 dark:bg-emerald-900/30' : 'border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800'}`}>
                                                <div className="flex items-center gap-3 mb-2">
                                                    <button type="button" onClick={() => {
                                                        if (mode === 'SINGLE') setCorrectIndices([i]);
                                                        else setCorrectIndices(prev => prev.includes(i) ? prev.filter(x => x !== i) : [...prev, i]);
                                                    }} className="w-8 h-8 rounded-lg bg-white dark:bg-slate-700 border dark:border-slate-600 flex items-center justify-center font-black dark:text-white">{['A', 'B', 'C', 'D'][i]}</button>
                                                    <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase">Επιλογή {i + 1}</span>
                                                </div>
                                                {isImage && <img src={opt} className="h-32 w-full object-cover rounded-xl mb-2 border dark:border-slate-600" />}
                                                <input value={opt} onChange={e => { const n = [...options]; n[i] = e.target.value; setOptions(n) }} className="w-full bg-transparent outline-none border-b border-dashed dark:border-slate-600 dark:text-white dark:placeholder-slate-500" placeholder="Κείμενο ή URL..." />
                                                <label className="absolute top-4 right-4 cursor-pointer text-slate-300 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"><Upload size={16} /><input type="file" hidden accept="image/*" onChange={async (e) => { if (e.target.files?.[0]) { const url = await handleFileUpload(e.target.files[0]); const n = [...options]; n[i] = url; setOptions(n); } }} /></label>
                                            </div>
                                        )
                                    })}
                                </div>
                                ...
                                {pairs.map((p, i) => (
                                    <div key={i} className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            <input value={p.left} onChange={e => { const n = [...pairs]; n[i].left = e.target.value; setPairs(n) }} className="w-full p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border dark:border-slate-700 dark:text-white dark:placeholder-slate-500" placeholder="Αριστερό τμήμα" />
                                            <div className="flex gap-2"><input value={p.leftImg || ""} onChange={e => { const n = [...pairs]; n[i].leftImg = e.target.value; setPairs(n) }} className="flex-1 text-xs bg-transparent outline-none dark:text-slate-400 dark:placeholder-slate-600" placeholder="URL (opt)" /></div>
                                        </div>
                                        <div className="space-y-1">
                                            <input value={p.right} onChange={e => { const n = [...pairs]; n[i].right = e.target.value; setPairs(n) }} className="w-full p-3 bg-emerald-50 dark:bg-emerald-900/30 rounded-xl border border-emerald-100 dark:border-emerald-800 dark:text-white dark:placeholder-slate-500" placeholder="Δεξί τμήμα" />
                                            <div className="flex gap-2 items-center">
                                                <input value={p.rightImg || ""} onChange={e => { const n = [...pairs]; n[i].rightImg = e.target.value; setPairs(n) }} className="flex-1 text-xs bg-transparent outline-none dark:text-slate-400 dark:placeholder-slate-600" placeholder="URL (opt)" />
                                                <label className="cursor-pointer text-slate-300 dark:text-slate-500 hover:text-emerald-500 dark:hover:text-emerald-400"><Upload size={12} /><input type="file" hidden onChange={async (e) => { if (e.target.files?.[0]) { const u = await handleFileUpload(e.target.files[0]); const n = [...pairs]; n[i].rightImg = u; setPairs(n); } }} /></label>
                                            </div>
                                            {p.rightImg && <img src={p.rightImg} className="h-10 rounded-lg border dark:border-slate-600 object-cover" />}
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="space-y-8">
                                <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs font-bold uppercase text-slate-500 dark:text-slate-400">Λειτουργία:</span>
                                        <span className={`text-xs font-black px-2 py-1 rounded ${isInlineMode ? 'bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300' : 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300'}`}>
                                            {isInlineMode ? "INLINE (Επιλογές για κάθε ένα)" : "GLOBAL (Τράπεζα λέξεων)"}
                                        </span>
                                    </div>
                                    <button type="button" onClick={() => setIsInlineMode(!isInlineMode)} className="text-xs font-bold text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white underline">Αλλαγή</button>
                                </div>

                                <div className="space-y-2">
                                    <textarea value={fillSentence} onChange={e => setFillSentence(e.target.value)} rows={4} className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border dark:border-slate-700 outline-none dark:text-white dark:placeholder-slate-500" placeholder="Κείμενο με κενά (1), (2)..." />
                                </div>

                                {!isInlineMode && (
                                    <div className="space-y-2 bg-slate-50 dark:bg-slate-800 p-4 rounded-xl">
                                        <label className="text-xs font-black uppercase text-slate-400 dark:text-slate-500">Τράπεζα λέξεων</label>
                                        <div className="flex flex-wrap gap-2">
                                            {fillOptions.map((opt, i) => (
                                                <div key={i} className="flex items-center gap-1 bg-white dark:bg-slate-700 border dark:border-slate-600 px-3 py-2 rounded-lg">
                                                    <span className="font-bold text-sm dark:text-white">{opt}</span>
                                                    <button type="button" onClick={() => setFillOptions(prev => prev.filter((_, idx) => idx !== i))} className="text-red-400"><X size={14} /></button>
                                                </div>
                                            ))}
                                            <input className="px-3 py-2 rounded-lg border border-dashed dark:border-slate-600 bg-transparent outline-none text-sm min-w-[150px] dark:text-white dark:placeholder-slate-500" placeholder="+ Enter" onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); const val = e.currentTarget.value.trim(); if (val) { setFillOptions([...fillOptions, val]); e.currentTarget.value = ""; } } }} />
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    {getFillKeys(fillSentence).map((key) => (
                                        <div key={key} className="p-4 border dark:border-slate-700 rounded-xl flex flex-col gap-2">
                                            <span className="text-xs font-bold uppercase text-slate-400 dark:text-slate-500">Κενό {key}</span>
                                            {isInlineMode && (
                                                <input
                                                    value={(inlineChoices[key] || []).join(", ")}
                                                    onChange={(e) => setInlineChoices({ ...inlineChoices, [key]: e.target.value.split(",").map(s => s.trim()) })}
                                                    className="w-full p-2 bg-slate-50 dark:bg-slate-800 rounded text-sm border-none outline-none dark:text-white dark:placeholder-slate-500"
                                                    placeholder="Επιλογές χωρισμένες με κόμμα..."
                                                />
                                            )}
                                            <select
                                                value={fillCorrectMap[key] || ""}
                                                onChange={(e) => setFillCorrectMap({ ...fillCorrectMap, [key]: e.target.value })}
                                                className="w-full p-2 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300 font-bold rounded outline-none dark:border dark:border-emerald-800"
                                            >
                                                <option value="">-- Σωστή απάντηση --</option>
                        ...
                        {mode === 'TRUE_FALSE' && (
                            <div className="space-y-3">
                                {tfItems.map((item, i) => (
                                    <div key={i} className="flex flex-col md:flex-row md:items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
                                        <div className="flex-1 space-y-2">
                                            <input value={item.text || ""} onChange={e => { const n = [...tfItems]; n[i].text = e.target.value; setTfItems(n) }} className="w-full bg-transparent outline-none p-2 font-medium dark:text-white dark:placeholder-slate-500" placeholder="Δήλωση..." />
                                            <div className="flex items-center gap-2">
                                                <input value={item.imageUrl || ""} onChange={e => { const n = [...tfItems]; n[i].imageUrl = e.target.value; setTfItems(n) }} className="flex-1 text-xs bg-transparent outline-none text-slate-400 dark:text-slate-500 dark:placeholder-slate-600" placeholder="URL εικόνας..." />
                                                <label className="cursor-pointer text-slate-300 dark:text-slate-500 hover:text-pink-500 dark:hover:text-pink-400"><Upload size={14} /><input type="file" hidden onChange={async (e) => { if (e.target.files?.[0]) { const u = await handleFileUpload(e.target.files[0]); const n = [...tfItems]; n[i].imageUrl = u; setTfItems(n); } }} /></label>
                                            </div>
                                            {item.imageUrl && <img src={item.imageUrl} className="h-20 rounded-lg border dark:border-slate-600 object-cover" />}
                                        </div>
                                ...
                                <button type="button" onClick={() => setTfItems([...tfItems, { id: Math.random().toString(), text: "", isTrue: true }])} className="text-xs font-bold text-blue-500 dark:text-blue-400">+ Προσθήκη</button>
                            </div>
                        )}

                        {mode === 'OPEN' && (
                            <textarea value={modelAnswer} onChange={e => setModelAnswer(e.target.value)} className="w-full p-4 bg-emerald-50/30 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-xl outline-none dark:text-white dark:placeholder-slate-500" placeholder="Πρότυπο..." />
                        )}
                    </div>

                    <button disabled={isSaving || isUploading} className="w-full py-5 bg-slate-900 dark:bg-blue-600 text-white rounded-[2rem] font-black text-xl shadow-xl flex items-center justify-center gap-3 hover:bg-slate-800 dark:hover:bg-blue-700 transition-colors">
                ...
                <div className="mt-10 pt-10 border-t border-slate-200 dark:border-slate-700">
                    <button onClick={() => setShowDebug(!showDebug)} className="flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-slate-900 dark:hover:text-white"><Bug size={14} /> Debug</button>
