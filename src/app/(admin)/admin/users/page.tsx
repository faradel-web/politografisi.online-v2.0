    ...
    return (
        <div className="p-6 md:p-8 max-w-7xl mx-auto font-sans">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    <h1 className="text-3xl font-black flex items-center gap-3 font-serif text-slate-900 dark:text-white">
                        Διαχείριση Χρηστών
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium mt-1">Διαχείριση προσβάσεων και συνδρομιών</p>
                </div>
                ...
                <div className="relative w-full md:w-72">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-3 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all shadow-sm font-medium"
                    />
                </div>
            </div>

            {/* TABLE */}
            <div className="bg-white dark:bg-slate-900 rounded-[1.5rem] shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden min-h-[400px]">
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 text-xs uppercase text-slate-400 font-bold tracking-wider">
                            <tr>
                        ...
                        <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                            {filteredUsers.map((user) => (
                                <tr key={user.id} className="hover:bg-indigo-50/30 dark:hover:bg-slate-800/50 transition-colors group">
                                    <td className="p-5">
                                        <div className="font-bold text-slate-900 dark:text-slate-100 text-base">{user.displayName || "Χωρίς όνομα"}</div>
                                        <div className="text-xs text-slate-400 font-mono mt-0.5 select-all">{user.id.substring(0, 8)}...</div>
                                    </td>
                                    <td className="p-5">
                                        <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400 mb-1 font-medium">
                                    ...
                                    <td className="p-5 text-slate-500 dark:text-slate-400 font-medium">
                                        {user.role === USER_ROLES.STUDENT && user.subscriptionEndsAt ? (
                                            <span className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800 px-3 py-1 rounded-lg border border-slate-100 dark:border-slate-700 w-fit text-slate-600 dark:text-slate-300">
                                    ...
                                    <td className="p-5 text-right">
                                        <button
                                            ...
                                            className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-indigo-50 dark:hover:bg-slate-700 hover:border-indigo-200 dark:hover:border-indigo-600 text-slate-400 hover:text-indigo-600 p-2.5 rounded-xl transition-all shadow-sm group-hover:opacity-100"
                                            title="Επεξεργασία"
                                        >
            ...
            {editingUser && (
                <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm transition-all">
                    <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-lg p-8 shadow-2xl animate-in zoom-in-95 duration-200 border border-slate-100 dark:border-slate-700">

                        <h2 className="text-2xl font-black mb-6 text-slate-900 dark:text-white font-serif">Επεξεργασία Χρήστη</h2>

                        {/* User Info */}
                        <div className="mb-8 bg-slate-50 dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700 flex items-start gap-4">
                            <div className="h-12 w-12 rounded-full bg-white dark:bg-slate-700 border border-indigo-100 dark:border-indigo-700 flex items-center justify-center text-indigo-700 dark:text-indigo-400 font-black text-xl shadow-sm">
                                {editingUser.displayName?.charAt(0) || "U"}
                            </div>
                            <div>
                                <p className="font-bold text-slate-900 dark:text-white text-lg">{editingUser.displayName}</p>
                                <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">{editingUser.email}</p>
                        ...
                        <div className="space-y-6 mb-8">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <select
                                        ...
                                        className="w-full p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 appearance-none font-bold text-slate-700 dark:text-slate-200 shadow-sm"
                                    >
                            ...
                            {selectedRole === USER_ROLES.STUDENT && (
                                <div className="bg-indigo-50 dark:bg-indigo-950/30 p-6 rounded-2xl border border-indigo-100 dark:border-indigo-800 animate-in slide-in-from-top-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full p-3 border border-indigo-200 dark:border-indigo-700 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 mb-4 bg-white dark:bg-slate-800 font-bold text-indigo-900 dark:text-indigo-300 shadow-sm"
                                    />
                        ...
                        {/* Footer */}
                        <div className="flex gap-3 justify-end pt-4 border-t border-slate-50 dark:border-slate-800">
                            <button
                            ...
                            <button
                                onClick={handleSave}
                                disabled={isSaving}
                                className="px-8 py-3 bg-slate-900 dark:bg-indigo-700 text-white font-bold rounded-xl hover:bg-black dark:hover:bg-indigo-800 transition-colors shadow-lg disabled:opacity-70 flex items-center gap-2"
                            >
...
const QuickButton = ({ label, onClick, highlight }: { label: string, onClick: () => void, highlight?: boolean }) => (
    <button
        onClick={onClick}
        className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all border ${highlight
