...
export default function ExamWelcomePage() {
    ...
    if (accessDenied) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center text-red-500 font-bold bg-slate-50 dark:bg-slate-950/50 p-6 text-center">
                <ShieldCheck size={48} className="mb-4 opacity-50" />
                <h2 className="text-2xl font-black mb-2">Όριο Εξετάσεων</h2>
                <p className="text-slate-600 dark:text-slate-400 font-medium">Έχετε φτάσει το όριο των δωρεάν προσπαθειών. Αναβαθμίστε σε Premium.</p>
            </div>
        );
    }

    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950/50 p-4 sm:p-8 flex items-center justify-center font-sans">
            <div className="bg-white dark:bg-slate-900 max-w-4xl w-full p-8 sm:p-12 rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-slate-800 flex flex-col md:flex-row gap-12">
                ...
                <div className="flex-1 space-y-8">
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 font-medium">Πλήρης προσομοίωση για την πιστοποίηση ελληνομάθειας.</p>
                    </div>

                    <div className="space-y-4">
                        <div className="bg-slate-50 dark:bg-slate-950/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800 flex items-start gap-3">
                            <Clock className="text-blue-600 shrink-0 mt-1" size={20} />
                            <div>
                                <h4 className="font-bold text-slate-900 dark:text-white text-sm">Διάρκεια: 3 Ώρες</h4>
                                <p className="text-xs text-slate-500 dark:text-slate-400">Ο χρόνος μετράει αντίστροφα μόλις πατήσετε "Έναρξη".</p>
                            </div>
                        </div>
                        <div className="bg-slate-50 dark:bg-slate-950/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800 flex items-start gap-3">
                            <BrainCircuit className="text-purple-600 shrink-0 mt-1" size={20} />
                            <div>
                                <h4 className="font-bold text-slate-900 dark:text-white text-sm">AI Αξιολόγηση</h4>
                                <p className="text-xs text-slate-500 dark:text-slate-400">Άμεση βαθμολόγηση Έκθεσης & Προφορικών από το Gemini AI.</p>
                            </div>
                        </div>
                    </div>
                ...
                {/* RIGHT: STRUCTURE */}
                <div className="flex-1 bg-slate-900 dark:bg-slate-950 rounded-[2rem] p-8 text-white relative overflow-hidden flex flex-col justify-between">
                    ...
                    <div>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center gap-4 p-3 bg-white/5 dark:bg-slate-800/50 rounded-xl border border-white/10 dark:border-white/5 hover:bg-white/10 dark:hover:bg-slate-800/80 transition-colors">
