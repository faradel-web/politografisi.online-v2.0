...
export default function StudyHubPage() {
  ...
  const CategoryCard = ({ cat }: { cat: typeof CATEGORIES[0] }) => {
    ...
    const theoryHref = `/theory/${cat.id}`; // unused but keeping variables safe
    const practiceHref = `/practice/${cat.id}`;
    ...
    return (
      <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col justify-between h-full hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
        ...
        {/* КНОПКИ: Тільки Εξάσκηση */}
        <div className="grid grid-cols-1 mt-auto">
          <Link
            href={practiceHref}
            className={`flex items-center justify-center gap-2 py-3 px-2 rounded-xl font-bold text-sm transition-all shadow-sm ${practiceCount > 0 ? 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-emerald-200' : 'bg-slate-100 dark:bg-slate-800 text-slate-300 cursor-not-allowed'}`}
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950/50 p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-12">
        <header className="mb-8">
          <h1 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white mb-2 tracking-tight">Εξάσκηση</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium text-base md:text-lg">Επιλέξτε κατηγορία για εξάσκηση</p>
        </header>
        ...
        <section>
          <div className="flex items-center gap-3 mb-6 px-2">
            ...
            <h2 className="text-lg md:text-xl font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest">Γνώσεις (Ιστορία/Πολιτικοί Θεσμοί/Γεωγραφία/Πολιτισμός)</h2>
          </div>
        ...
        <section>
          <div className="flex items-center gap-3 mb-6 px-2">
            ...
            <h2 className="text-lg md:text-xl font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest">Ελληνομάθεια (Γλώσσα)</h2>
          </div>
